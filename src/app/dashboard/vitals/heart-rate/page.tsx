...
export default function HeartRateClient() {
  ...
  /* Load & filter */
  useEffect(() => {
    const vitals = getVitals().filter((v) => v.type === "heart-rate")
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <SummaryCard label="Average" value={`${avg.toFixed(1)} bpm`} />
        <SummaryCard label="Minimum" value={`${min} bpm`} />
        <SummaryCard label="Maximum" value={`${max} bpm`} />
      </div>
