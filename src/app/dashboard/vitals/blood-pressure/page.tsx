...
export default function BloodPressureClient() {
  ...
  useEffect(() => {
    const vitals = getVitals().filter(
      (v) => v.type === "blood-pressure"
    )
    setData(filterByRange(vitals, range))
  }, [range])

  /* Graph-safe formatted data */
  ...
  /* Summary stats */
  const avgSystolic =
    chartData.reduce((a, b) => a + b.systolic, 0) /
    (chartData.length || 1)

  const avgDiastolic =
    chartData.reduce((a, b) => a + b.diastolic, 0) /
    (chartData.length || 1)
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <SummaryCard
          ...
          value={`${avgSystolic.toFixed(1)} mmHg`}
        />
        <SummaryCard
          label="Average Diastolic"
          value={`${avgDiastolic.toFixed(1)} mmHg`}
        />
      </div>
