...
export default function WeightClient() {
  const [range, setRange] = useState<number>(30) // weight usually monthly
  const [open, setOpen] = useState(false)
  const [data, setData] = useState<VitalEntry[]>([])

  /* Load & filter weight data */
  useEffect(() => {
    const vitals = getVitals().filter((v) => v.type === "weight")
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <SummaryCard label="Average" value={`${avg.toFixed(1)} kg`} />
        <SummaryCard label="Minimum" value={`${min} kg`} />
        <SummaryCard label="Maximum" value={`${max} kg`} />
      </div>
