...
useEffect(() => {
  ...
  const calculateStats = (reports: Report[]) => {
    ...
    reports.forEach(report => {
      // Count by type
      byType[report.typeLabel] = (byType[report.typeLabel] || 0) + 1
  ...
  const handleFilter = (filters: any) => {
    ...
    if (filters.vital) {
      // Filter by extracted vital parameter
      filtered = filtered.filter(report => {
        if (!report.extractedData) return false
        // Check if this report has the selected vital
        return Object.keys(report.extractedData).some(key => 
          key.toLowerCase().includes(filters.vital.toLowerCase())
        )
      })
    }
